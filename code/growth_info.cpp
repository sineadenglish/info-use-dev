...
// mortality as function of alpha in good environment
double pdead_good(double alpha, double &MU0, double &MU1_GOOD, double &GAMMA)
{
	return MU0*(1.0 + MU1_GOOD*pow(alpha,GAMMA));
}

// mortality as function of alpha in bad environment
double pdead_bad(double alpha, double &MU0, double &MU1_BAD, double &GAMMA)
{
	return MU0*(1.0 + MU1_BAD*pow(alpha,GAMMA));
}


// Initialise values (here at random values, but could also initialize at zero or fitness at maturity)
...
void Initialise() 
{
	...
	{ 
      ...
      new_value[size_val][p_val] = 0.0;    
			...
			{
				fitAlphaArray[size_val][p_val][alpha_i] = 0.0/0;  // so NaN 
			}
		} 
	} 
}
	...
	{  
        ...
        for (alpha_i = 0; alpha_i < ALPHASTEPS; alpha_i++)
				{					
					...
					// probability of survival given particular alpha and environment
					mu_good = (1.0 - pdead_good(alpha, MU0, MU1_GOOD, GAMMA));  
					mu_bad = (1.0 - pdead_bad(alpha, MU0, MU1_BAD, GAMMA));  
	...
	{  
        ...
        best_alpha = 0.0;
				...
				for (alpha_i = 0; alpha_i < ALPHASTEPS; alpha_i++)
				{
					...
					// probability of death given that alpha and environment 
					mu_good = (1.0 - pdead_good(alpha, MU0, MU1_GOOD, GAMMA));  
					mu_bad = (1.0 - pdead_bad(alpha, MU0, MU1_BAD, GAMMA));  
...
void Val_Info_Grow(double &PHI, double &KAPPA_GOOD, double &KAPPA_BAD, double &CONST_GOOD, double &CONST_BAD, double &MU0, double &MU1_GOOD, double &MU1_BAD, double &GAMMA, double &PFOOD_GOOD, double &PFOOD_BAD)
{
	...
	for(size_val=0; size_val<SIZESTEPS; size_val++)
	{
		...
		for(p_val=0;p_val<PSTEPS; p_val++)
		{
			...
			{
				// probability of survival given that alpha and environment 
				survGood = (1.0 - pdead_good(alpha_size0_pgood0, MU0, MU1_GOOD, GAMMA));
				survBad = (1.0 - pdead_bad(alpha_size0_pgood0, MU0, MU1_BAD, GAMMA));
